...
class SiblingMenuBlock extends BlockBase
{
  ...
  public function build()
  {
    ...
    /** @var \Drupal\sibling_menu_block\Plugin\Block\SiblingMenuBlock $wholeMenuData  Formed by us the whole vocabulary with deep, determined loadTree parameters */
    $wholeMenuData = [];
    ...
    foreach ($tree as $key => $term) {
      ...
      /** @var \Drupal\sibling_menu_block\Plugin\Block\SiblingMenuBlock  $parents array of parents */
      ...
      foreach ($term->getFields(true) as $field) {
        ...
        if ($field->getName() == 'field_products_category_picture') {
          if (isset($field->getValue()[0]['target_id'])) {
            ...
            $file_url = \Drupal::service('file_url_generator')->generateAbsoluteString($file->getFileUri()); // get the URL (D10).
    ...
    foreach ($wholeMenuData as $key => $field) {
      if ($field['depth'] < $depth) {
        //dpm($field['id'] . ' is not my sibling and not me!');
